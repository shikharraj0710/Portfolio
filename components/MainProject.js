  ...
  return (
    <>
      ...
      >
        ...
        <div className="text-xs mb-3 font-medium">{new Date(data.duration).toLocaleDateString('en-ES', {day : "numeric", weekday : "long", year :"2-digit", month : "long"})}</div>
        <ul className="text-sm md:text-md list-disc marker:text-darkGolden capitalize list-inside mb-3">
          {data?.desc[0].split(".")?.map((item, i) => (
