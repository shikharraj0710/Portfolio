...
function experienceOperationIndex({ data, length }) {
    const dataState = useMemo(() => data, [data]);
    return (
        <>
            <Head><title>Experiences</title></Head>
            <div className="min-h-[70vh] my-16">
                <div className='grid grid-cols-2 items-center mb-8 w-4/5 mx-auto'>
                    ...
                    <button className=''>
                        <a className=' ml-auto text-right border-2 border-solid border-darkGolden rounded-md px-5 py-1 uppercase text-customBlack tracking-wider font-medium transition-all focus:outline-none  hover:text-white hover:font-medium hover:bg-[#eba352] ' href='/admin/experienceOperation/addExperience'>Add</a>
                    </button>
                </div>
...
export default experienceOperationIndex;

export async function getServerSideProps() {

    const res = await fetch(`${server}/api/experience`, {
        method: "GET",
        headers: {
